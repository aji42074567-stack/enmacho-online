...
| 10 | 紫怨の喪衣 | 伝説 | `armor_shion_mourning.png` | 使用中 (`shion_mofuku`・ガシャオウ) |
...
| 5 | 骸面の頭巾 | 希少 | `head_gaimen_hood.png` | 使用中 (`gaimen_hood`・無縁塚オオドクロ) |
...
| 8 | 獣骨の呪籠手 | 名品 | `arms_jukotsu_noroi.png` | 使用中 (`jukotsu_kote`・無縁塚キョウコツ) |
...
| 10 | 砂葬の数珠足袋 | 上等 | `feet_saso_juzu_tabi.png` | 使用中 (`saso_tabi`・無縁塚シラホネ) |
...
| 4 | 浄めの匂袋 | 希少 | `acc_kiyome_nioibukuro.png` | 使用中 (`kiyome_bukuro`・クエスト「骨の王の鎮魂」報酬) |
...
| 名称 | ファイル | 用途 |
|---|---|---|
| 武器強化の巻物 | `scroll_weapon.png` | 装備中の武器を+1(カナヤマで使用。失敗しても+値は維持・上限なし) |
| 防具強化の巻物 | `scroll_armor.png` | 装備中の胴を+1(カナヤマで使用。失敗しても+値は維持・上限なし) |
| 冥職やり直しの札 | CSS表示 | 運営からの個別付与品。現在の冥職を解除し、選択画面から選び直す |
| 性別変更の札 | CSS表示 | GM専用。魂の姿を男性・女性の間で切り替える |
| 神足の御符 | `acc_kifuda_gofu.png` | GM専用。1個消費して10分間、移動速度を2.6倍にする |

これらの札・御符は通常セーブに所持数を保存せず、`gm_item_inventory`で管理する。
在庫テーブルはクライアントから直接参照・更新できず、「冥職やり直しの札」の表示と消費のRPCは
ログイン中の魂籍に個別付与された在庫だけを扱う。「性別変更の札」は表示と消費の両方で
`admin_users`のGM権限を検査する。「神足の御符」も表示・消費の両方でGM権限を検査し、
効果中はHUDへ残り時間を表示する。個別魂籍への付与記録は公開リポジトリへ保存しない。

## 希少素材

| 名称 | 入手先 | 確率 | 所持形式 | 現在の用途 |
|---|---|---:|---|---|
| 竜の血 | ゴウリュウ | 7% | スタック可能 | 移し打ち(+値の引っ越し)の素材。鍛冶屋カナヤマで血3個+手数料(帯の時給50%のモン) |
