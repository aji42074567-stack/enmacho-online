...
| 冥職やり直しの札 | CSS表示 | GM専用。現在の冥職を解除し、選択画面から選び直す |
| 性別変更の札 | CSS表示 | GM専用。魂の姿を男性・女性の間で切り替える |

GM専用の札は通常セーブに所持数を保存せず、`gm_item_inventory`で管理する。
表示と消費の両方で`admin_users`のGM権限を検査する。GMの識別につながるため、
個別魂籍への付与記録は公開リポジトリへ保存しない。
